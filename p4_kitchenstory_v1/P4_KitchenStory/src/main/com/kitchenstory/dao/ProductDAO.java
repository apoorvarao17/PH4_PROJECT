...
public class ProductDAO {
	...
	public List<Product> getAllProducts() {
		...
		System.out.println("Entered DAO get all products");
		return list;
	}
	
	@SuppressWarnings("unchecked")
	public List<Product> getProductByItemName(String iName) {
		List<Product> list = this.sessionFactory.getCurrentSession().createQuery("from Product where name LIKE '%" + iName + "%'").list();
		if (list.size() > 0)
			return list;
		else
			return null;
